...
import { useUpdateAdsMutation } from "../../app/services/manageApi";
...
const EditAdsDialog = ({
  ...
  const [imageUpload, setImageUpload] = useState(ADimage);
  ...
  const [owner, setOwner] = useState(ADowner);
  const [contact, setContact] = useState(ADcontact);
  const [email, setEmail] = useState(ADemail);
